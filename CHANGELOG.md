...
### Improvements
* Added `MemoryGame.js` files for blueprints for convenient imports
...
### Improvements
* Renamed application entry point from `MemoryGame.js` -> `app.js` (clarifies intent and helps with coverage reports)
...
* `reducers/MemoryGame.js` now exports combined root reducer.
