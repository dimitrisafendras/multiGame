...
describe('Users', () => {
  ...
  describe('POST /users', () => {
    it('should reject signup when data is incomplete', done => {
      request.post('/users').set('Accept', 'application/json').send({ username: 'supercoolname' }).expect(422, done);
    });

    it('should sign up', done => {
      request.post('/users').set('Accept', 'application/json').send({ user: { username: 'supercoolname', password: 'supersecretpassword' } }).expect(200, (err, res) => {
        if (err) {
          return done(err);
        }
  ...
  describe('GET /users', () => {
    it('should not fetch users if the authorization header is missing', done => {
      request.get('/users').set('Accept', 'application/json').expect(401, done);
    });
